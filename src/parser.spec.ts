...
describe('Parse', () => {
  describe('Parse', () => {
    const formats = [
      ...
      { input: 'N41 25.117', expected: { signal: '+', compass: Compass.NORTH, degrees: 41, minutes: 25.117 } },
      { input: '-41 25.117', expected: { signal: '-', compass: undefined, degrees: 41, minutes: 25.117 } },
      { input: "-41°25'", expected: { signal: '-', compass: undefined, degrees: 41, minutes: 25, seconds: 0 } },
      ...
      { input: 'N41.092', expected: { signal: '+', compass: Compass.NORTH, degrees: 41.092 } },
      { input: '+N41.092', expected: { signal: '+', compass: Compass.NORTH, degrees: 41.092 } },
      ...
      // Decimal
      { input: 123.456789, expected: { signal: '+', compass: undefined, degrees: 123.456789 } },
      { input: -123.456789, expected: { signal: '-', compass: undefined, degrees: 123.456789 } },
    ];

    test.each(formats)('DMS %s', (data) => {
      const parse = new Parser(data.input);
      console.log(data.input, parse.toJSON());
      expect(parse).toMatchObject(data.expected)
    });
  });
});
